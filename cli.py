"""命令行交互界面：输入文档路径 → AI 生成大纲/布局 → 渲染 PPT。
...
def run_once(file_path: str):
    ...
    if ask_yes_no("是否查看大纲 JSON", default=False):
    ...
    if ask_yes_no("是否查看布局 JSON", default=False):
        print(layout_info.model_dump_json(indent=2))
